// raymarcher.cpp : définit le point d'entrée de l'application.
...
bool hit(ray r, float tmin, float tmax, hitRecord rec, hitableList list)
{
    hitRecord tempRec;
    bool hitAny = false;
    ...
    {
        if(hitSphere(r,tmin, closestSoFar,tempRec, list.list[i]))
        {
            hitAny = true;
...
vec3 color(ray r, hitableList list, vec2 st, int sample)
{
    hitRecord rec;
    vec3 unitDirection;
    float t;
...
void mainImage(vec4 fragColor, vec2 fragCoord, int maxSample, vec2 resolution, vec2 coord)
{
    vec2 st = coord/resolution.xy;
    camera cam;
    cam.llc = vec3(-2.,-1.,-1.);
    cam.h = vec3(4.,0.,0.);
    cam.v = vec3(0.,2.25,0.);
